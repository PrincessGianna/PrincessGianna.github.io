...
function initializeMusicPlayer() {
  ...
  musicPlayer = {
    ...
    currentSong: 0,
    songs: [
      { title: 'Beautiful Love Song', artist: 'Romantic Artist', duration: 225 },
...
function playPreviousSong() {
  ...
  musicPlayer.currentSong = (musicPlayer.currentSong - 1 + musicPlayer.songs.length) % musicPlayer.songs.length;
  musicPlayer.currentTime = 0;
  updateMusicPlayerDisplay();
  console.log('Playing previous song:', musicPlayer.songs[musicPlayer.currentSong].title);
}
...
function playNextSong() {
  ...
  musicPlayer.currentSong = (musicPlayer.currentSong + 1) % musicPlayer.songs.length;
  musicPlayer.currentTime = 0;
  updateMusicPlayerDisplay();
  console.log('Playing next song:', musicPlayer.songs[musicPlayer.currentSong].title);
}
...
function handleTagClick(tag) {
  ...
  // 这里可以打开模态框显示详细信息
  showTagModal(tagText);
}

/**
 * 显示标签模态框
 */
function showTagModal(tagText) {
  const modal = utils.$('#tag-modal');
  if (!modal) return;

  const title = utils.$('#tag-detail-title');
  const description = utils.$('#tag-detail-description');

  if (title) title.textContent = tagText;
  if (description) {
    description.textContent = `关于"${tagText}"的美好回忆和详细描述...`;
  }

  utils.addClass(modal, 'show');

  // 绑定关闭事件
  const closeBtn = utils.$('#tag-modal-close');
  const detailCloseBtn = utils.$('#tag-detail-close-btn');

  const closeModal = () => utils.removeClass(modal, 'show');
  ...
  if (detailCloseBtn) {
    detailCloseBtn.onclick = closeModal;
  }
  ...
  statCards.forEach((card, index) => {
    ...
    setTimeout(() => {
      if (targetValue === '∞') {
        ...
        utils.animateNumber(0, parseInt(targetValue), 2000, (current, isComplete) => {
...
function initializeScrollAnimations() {
  const animatedElements = utils.$$('.fade-in-up, .slide-in-left, .slide-in-right, .scale-in');
