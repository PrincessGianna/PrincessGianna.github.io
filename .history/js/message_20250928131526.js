...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // 手写动画控制 - 逐字打字机效果（适配翻页书）
  function startHandwritingAnimation(pageIndex = 1) {
    // 如果正在动画中，先停止
    if (isAnimating) {
      stopAllAnimations();
    }
    ...
    // 获取当前页的手写元素
    const currentPageElement = flipbook.querySelector(`.page:nth-child(${pageIndex}) .love-page`);
    if (!currentPageElement) return;
    
    const handwritingLines = currentPageElement.querySelectorAll(".handwriting-line");
  ...
  function resetHandwritingAnimation() {
    ...
    // 重置所有页面的手写行
    const allPages = flipbook.querySelectorAll('.page .love-page');
  ...
  function initializeFlipbook() {
    if (!flipbook || typeof $ === 'undefined' || !$.fn.turn) {
      console.warn('Turn.js 未正确加载');
      return;
    }
    ...
    try {
      ...
      flipbookInstance = $(flipbook).turn({
        ...
        when: {
          ...
          turned: function(event, page, view) {
            ...
            // 翻页完成后开始新页面的打字动画
            setTimeout(() => {
              startHandwritingAnimation(page);
            }, 300);
          }
        }
      });
      ...
      console.error('初始化翻页书失败:', error);
    }
  }

  // 重置翻页书
  function resetFlipbook() {
    if (flipbookInstance && flipbook) {
      try {
  ...
  if (prevButton) {
    prevButton.addEventListener('click', function() {
      if (flipbookInstance && currentPage > 1) {
  ...
  if (nextButton) {
    nextButton.addEventListener('click', function() {
      if (flipbookInstance && currentPage < totalPages) {
        $(flipbook).turn('next');
      }
    });
  }
  ...
  // 键盘导航支持
  document.addEventListener('keydown', function(e) {
    if (modal.classList.contains('active') && flipbookInstance) {
  ...
  // 添加心跳动画给所有签名
  function addSignatureAnimation() {
  ...
  // 初始化签名动画
  setTimeout(addSignatureAnimation, 1000);
});
