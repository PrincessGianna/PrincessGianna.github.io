...
function initializeImageData() {
  // 从DOM中提取图片信息
  galleryState.images = Array.from(galleryElements.galleryItems).map((item, index) => {
    ...
    return {
      ...
      element: item,
      loaded: false,
      description: `这是一张拍摄于${date ? date.textContent : '某个特殊时刻'}的珍贵照片，记录了我们美好的回忆。`
    };
  });
...
function handleFilterClick(e) {
  ...
  console.log('Filter clicked:', filter, 'Current filter:', galleryState.currentFilter);
...
function applyFilter(filter) {
  ...
  console.log('Filtered images:', galleryState.filteredImages.length, 'out of', galleryState.images.length);
...
function animateFilterTransition() {
  ...
  galleryState.images.forEach((image, index) => {
    ...
    const shouldShow = galleryState.filteredImages.includes(image);
    ...
    if (shouldShow) {
      ...
      setTimeout(() => {
        if (item.style.opacity === '0') { // 确保没有被其他操作改变
...
function openModal(imageIndex) {
  const filteredIndex = galleryState.filteredImages.findIndex(img => img.id === imageIndex);
...
function loadModalImage() {
  const currentImage = galleryState.filteredImages[galleryState.currentImageIndex];
...
function updateModalNavigation() {
  ...
  const isLast = galleryState.currentImageIndex === galleryState.filteredImages.length - 1;
  ...
  const lazyLoadObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        const imageData = galleryState.images.find(img => img.element === item);
...
function bindImageEvents(imageData) {
  ...
  if (viewBtn) {
    viewBtn.addEventListener('click', (e) => {
      e.preventDefault();
      const imageIndex = galleryState.images.findIndex(img => img.id === imageData.id);
...
function removeImage(imageId) {
  ...
  galleryState.filteredImages = galleryState.filteredImages.filter(img => img.id !== imageId);
  galleryState.totalCount--;
...
function setFilter(filter) {
  const filterTab = Array.from(galleryElements.filterTabs).find(tab => tab.dataset.filter === filter);
  if (filterTab) {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => {
    // 检查是否在画廊页面
    if (window.location.pathname.includes('gallery.html') || window.location.pathname.includes('pages/gallery.html')) {
      setTimeout(() => {
        initializeGallery();
      }, 100);
    }
