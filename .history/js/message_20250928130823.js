...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function startHandwritingAnimation(pageNumber = currentPage) {
    ...
    handwritingLines.forEach((line) => {
      ...
      line.style.transform = "translateY(10px) scale(0.98)";
      line.style.filter = "blur(1px)";
      line.style.transition = "all 0.3s ease";
      
      // 保持原始高度，防止布局跳动
      if (!line.dataset.originalHeight) {
        line.dataset.originalHeight = line.offsetHeight + 'px';
      }
      line.style.minHeight = line.dataset.originalHeight;
