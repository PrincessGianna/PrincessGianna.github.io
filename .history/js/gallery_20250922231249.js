  ...
  function initializeImageData() {
    ...
    galleryState.images = Array.from(galleryElements.galleryItems).map(
      (item, index) => {
        ...
        console.log("Initialized image:", index, imageData.title, "Categories:", imageData.categories, "Year:", imageData.year);
        return imageData;
      }
    );
  ...
  function applyFilter(filter) {
    ...
    if (filter === "all") {
      ...
      galleryState.filteredImages = galleryState.images.filter((image) => {
        console.log("Checking image:", image.title, "Categories:", image.categories, "Year:", image.year, "Filter:", filter);
...
function animateFilterTransition() {
  ...
  console.log("Starting filter animation with", galleryState.filteredImages.length, "filtered images");

  galleryState.images.forEach((image, index) => {
    const item = image.element;
    if (!item) return;

    const shouldShow = galleryState.filteredImages.some(filteredImage => filteredImage.id === image.id);
