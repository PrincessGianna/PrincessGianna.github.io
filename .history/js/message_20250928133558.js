...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function resetHandwritingAnimation() {
    ...
    // 重置所有页面的手写行
    flipbook.find('.handwriting-line').each(function(index, line) {
      // 恢复原始文本但先清空
      line.textContent = "";
      ...
      // 确保移除任何可能残留的类
      $(line).removeClass("handwriting-shake");
    });
  }
  ...
  function initializeFlipbook() {
    if (isFlipbookInitialized || !flipbook.length) return;
  ...
  if (prevBtn) {
    prevBtn.addEventListener('click', function() {
      if (currentPage > 1 && flipbook.turn('is')) {
  ...
  if (nextBtn) {
    nextBtn.addEventListener('click', function() {
      if (currentPage < totalPages && flipbook.turn('is')) {
        flipbook.turn('next');
      }
    });
  }
  ...
  document.addEventListener('keydown', function(e) {
    ...
    if (e.key === 'ArrowLeft' && currentPage > 1 && flipbook.turn('is')) {
      e.preventDefault();
      flipbook.turn('previous');
    } else if (e.key === 'ArrowRight' && currentPage < totalPages && flipbook.turn('is')) {
      e.preventDefault();
      flipbook.turn('next');
    }
  });
