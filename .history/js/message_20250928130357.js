...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function addPageCurlEffect(currentPage, targetPage, direction) {
    // 为当前页面添加弯曲效果
    if (currentPage) {
      currentPage.style.transformOrigin = direction === 'forward' ? 'left center' : 'right center';
    }
    
    // 为目标页面设置初始状态
    if (targetPage) {
      targetPage.style.transformOrigin = direction === 'forward' ? 'left center' : 'right center';
    }
  }
